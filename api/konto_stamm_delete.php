...
if (isset($_REQUEST["kontonr"])) {
    ...
    if ($kontonr != "null" && $kontonr != "") {
        if (strlen($kontonr) > 50) {
            $out['response']['status'] = -4;
            $out['response']['errors'] =  "Bitte die kontonr prüfen!";
            print json_encode($out);
            return;
        }
    } else {
        $out['response']['status'] = -1;
        $out['response']['errors'] = "kontonr fehlt!";
        print json_encode($out);
        return;
    }
} else {
...
$sqlQuery = "Delete from konten where kontonr = ".$dbSyb->quote($kontonr) ;
