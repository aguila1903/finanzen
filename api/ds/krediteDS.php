...
$querySQL = " SELECT 
    ...
    dauer AS max_rate, 
    case when TIMESTAMPDIFF(month,ADDDATE(datum, INTERVAL -1 month),curdate()) >= dauer then dauer when TIMESTAMPDIFF(month,ADDDATE(datum, INTERVAL -1 month),CURDATE()) < 0 then 0 else TIMESTAMPDIFF(month,ADDDATE(datum, INTERVAL -1 month),curdate()) end as rate, 
